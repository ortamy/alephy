...
Поле `glyph` в реестре `LabHero.TARGETS` добавляет квадратный глиф-чип справа от текста шапки (палео-глиф, hairline `currentColor`, radius `sm`); эмодзи и растровые метки в шапке запрещены (раздел 3).
